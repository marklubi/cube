...
module.exports = function(options) {
  var server = {},
      primary = http.createServer(function (request, response) {
		if (options["isEvaluator"] != undefined && options["isEvaluator"] == true) {
			...
			request.on("end", function () {
            	...
            	var wait = 1000;
				for (var i = 0; i < endpoints.http.length; i++) {
			  		if ((endpoints.http[i]).match(u.pathname, request.method)) {
			    		var responses  = [];
			    		function callback(res) {
			      			//util.log(JSON.stringify(res));
			      			responses.push(res);
			    		}
			    		(endpoints.http[i]).dispatch(JSON.parse(body), callback);
			    		setTimeout(function() {
    ...
    for (var i = -1, n = endpoints.http.length, e; ++i < n;) {
      if ((e = endpoints.http[i]).match(u.pathname, request.method)) {
		if (request.method == "post") {
			//dump out on simple post requests so that on("data") can read the body
			return;
		}
